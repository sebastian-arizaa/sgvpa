...
interface MegaJsControladorInterface {
  ...
  conseguirTodosNombres(req: Request, res: Response): Promise<void>,
...
class MegaJsControlador implements MegaJsControladorInterface {
    ...
    if (req.sesion?.userTipo) {
      try {
        ...
        if (archvio) {
          ...
          res.status(404).send("Archivo no encontradoooooo")
        }
      } catch (error: any) {
  ...
  async conseguirTodosNombres(req: Request, res: Response): Promise<void> {
    if (req.sesion?.userTipo) {
      try {
